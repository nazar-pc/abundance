...
pub(crate) struct StorageItem {
    // NOTE: Sequence numbers for permanent and ephemeral storage items are independent
    pub(crate) sequence_number: u64,
    pub(crate) kind: StorageItemKind,
}

// TODO: Writes can't cross 1G boundary
impl StorageItem {
    const PAGE_GROUP_HEADER: u8 = 0;
    const BLOCK: u8 = 1;

    /// Returns the number of pages necessary to write this storage item
    pub(crate) fn num_pages(&self) -> u32 {
        let storage_item_size = match &self.kind {
        ...
        (Self::prefix_size() + storage_item_size + Self::suffix_size()).div_ceil(AlignedPage::SIZE)
            as u32
    }
    ...
    pub(crate) fn write_to_pages(
        ...
        let (enum_variant_bytes, storage_item_size_bytes) = remainder.split_at_mut(size_of::<u8>());

        // Write prefix
        sequence_number_bytes.copy_from_slice(&self.sequence_number.to_le_bytes());
        enum_variant_bytes[0] = Self::BLOCK;
    ...
    pub(crate) fn read_from_pages(pages: &[AlignedPage]) -> Result<Self, StorageItemError> {
        ...
        let (enum_variant_bytes, remainder) = remainder.split_at(size_of::<u8>());
        ...
        let enum_variant = enum_variant_bytes[0];
        ...
        match enum_variant {
            Self::PAGE_GROUP_HEADER => Ok(Self {
                sequence_number,
                kind: StorageItemKind::PageGroupHeader(StorageItemPageGroupHeader::read(
                    storage_item_bytes,
                )?),
            }),
            Self::BLOCK => Ok(Self {
                sequence_number,
                kind: StorageItemKind::Block(StorageItemBlock::read(storage_item_bytes)?),
            }),
            enum_variant => Err(StorageItemError::UnknownStorageItemVariant(enum_variant)),
        }
    }
}
