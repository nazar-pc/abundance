...
pub(crate) struct StorageItemBlock {
    ...
    pub(crate) mmr: Arc<BlockMerkleMountainRange>,
...
impl StorageItemBlock {
    ...
    pub(super) fn write(&self, mut buffer: &mut [u8]) -> Result<usize, StorageItemError> {
        ...
        // TODO: This is an extra memory copy, would be nice if header/body/etc. were aligned in
        //  such a way that allows to avoid some of these copies
        let mmr = self.mmr.as_bytes();

        let contents = [self.header.as_slice(), self.body.as_slice(), mmr.as_slice()];
        // Write all lengths
