use crate::shader::compute_fn::cpu_tests;
...
fn compute_fn_gpu() {
    ...
    let Some(actual_output) = block_on(compute_fn(chacha8_keystream.as_flattened(), num_x)) else {
    ...
    let expected_output = (0..num_x)
        .map(|x| UVec2 {
            x,
            y: cpu_tests::compute_fn::<{ PosProof::K }>(x, &seed),
        })
        .collect::<Vec<_>>();
...
async fn compute_fn(chacha8_keystream: &[u32], num_x: u32) -> Option<Vec<UVec2>> {
    ...
    for adapter in adapters {
        ...
        let adapter_result = compute_fn_adapter(chacha8_keystream, num_x, adapter).await?;
