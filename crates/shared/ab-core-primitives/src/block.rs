...
use crate::block::header::{
    BeaconChainBlockHeader, BlockHeader, GenericBlockHeader, IntermediateShardBlockHeader,
...
impl<'a> Deref for BeaconChainBlock<'a> {
...
impl<'a> BeaconChainBlock<'a> {
    ...
    pub fn try_from_bytes(bytes: &'a [u8]) -> Option<(Self, &'a [u8])> {
        ...
        // TODO: Check internal consistency of the block (that block matches body, etc.)
...
impl<'a> Deref for IntermediateShardBlock<'a> {
    type Target = GenericBlockHeader<'a>;
...
impl<'a> Deref for LeafShardBlock<'a> {
    type Target = GenericBlockHeader<'a>;
...
/// Block that contains [`BlockHeader`] and [`Body`]
...
impl<'a> Deref for Block<'a> {
    ...
    fn deref(&self) -> &Self::Target {
        match self {
            Self::BeaconChain(block) => block,
            Self::IntermediateShard(block) => block,
            Self::LeafShard(block) => block,
        }
    }
}
