...
/// Block header suffix.
///
/// The suffix contains summary information after the block was created.
#[derive(Debug, Copy, Clone, Eq, PartialEq, TrivialType)]
#[cfg_attr(
    feature = "scale-codec",
    derive(Encode, Decode, TypeInfo, MaxEncodedLen)
...
#[cfg_attr(feature = "serde", serde(rename_all = "camelCase"))]
#[repr(C)]
pub struct BlockHeaderSuffix {
    /// Root of the block body
    // TODO: New type
    pub block_body_root: Blake3Hash,
    /// Root of the state tree
    // TODO: New type?
    pub state_root: Blake3Hash,
}

impl BlockHeaderSuffix {
    /// Hash of the block header suffix, part of the eventual block hash
    pub fn hash(&self) -> Blake3Hash {
        // TODO: Keyed hash
        Blake3Hash::from(blake3::hash(self.as_bytes()))
    }
...
pub struct GenericBlockHeader<'a> {
    ...
    /// Block header suffix
    pub suffix: &'a BlockHeaderSuffix,
    /// Block header seal
...
impl<'a> BeaconChainBlockHeader<'a> {
    ...
    pub fn try_from_bytes(bytes: &'a [u8]) -> Option<(Self, &'a [u8])> {
        ...
        // * block header suffix: BlockHeaderSuffix
        // * block header seal: BlockHeaderSeal

        let (prefix, consensus_info, remainder) = BlockHeader::try_from_bytes_shared(bytes)?;

        let (child_shard_blocks, remainder) =
            BlockHeaderChildShardBlocks::try_from_bytes(remainder)?;

        let (consensus_parameters, mut remainder) =
            BlockHeaderBeaconChainParameters::try_from_bytes(remainder)?;

        let suffix = remainder.split_off(..size_of::<BlockHeaderSuffix>())?;
        // SAFETY: All bit patterns are valid
        let suffix = unsafe { BlockHeaderSuffix::from_bytes(suffix) }?;
...
impl<'a> IntermediateShardBlockHeader<'a> {
    ...
    pub fn try_from_bytes(bytes: &'a [u8]) -> Option<(Self, &'a [u8])> {
        ...
        let (prefix, consensus_info, mut remainder) = BlockHeader::try_from_bytes_shared(bytes)?;
        ...
        let (child_shard_blocks, mut remainder) =
            BlockHeaderChildShardBlocks::try_from_bytes(remainder)?;
...
impl<'a> BlockHeader<'a> {
    ...
    pub fn try_from_bytes(bytes: &'a [u8], shard_kind: ShardKind) -> Option<(Self, &'a [u8])> {
        match shard_kind {
            ShardKind::BeaconChain => {
                let (block_header, remainder) = BeaconChainBlockHeader::try_from_bytes(bytes)?;
                Some((Self::BeaconChain(block_header), remainder))
            }
            ShardKind::IntermediateShard => {
                let (block_header, remainder) =
                    IntermediateShardBlockHeader::try_from_bytes(bytes)?;
                Some((Self::IntermediateShard(block_header), remainder))
            }
            ShardKind::LeafShard => {
                let (block_header, remainder) = LeafShardBlockHeader::try_from_bytes(bytes)?;
    ...
    fn try_from_bytes_shared(
        ...
        Some((prefix, consensus_info, bytes))
    }
    ...
    pub fn hash(&self) -> BlockHash {
        ...
        match self {
            Self::BeaconChain(block_header) => block_header.hash(),
            Self::IntermediateShard(block_header) => block_header.hash(),
            Self::LeafShard(block_header) => block_header.hash(),
        }
    }
}
