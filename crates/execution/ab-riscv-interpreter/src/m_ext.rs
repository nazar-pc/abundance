    ...
    Registers: GenericRegisters64<Reg>,
{
    match instruction {
        ...
        MExtInstruction::Mulh { rd, rs1, rs2 } => {
            let (_lo, prod) = regs
                .read(rs1)
                .cast_signed()
        ...
        MExtInstruction::Divu { rd, rs1, rs2 } => {
        ...
        MExtInstruction::Rem { rd, rs1, rs2 } => {
            let dividend = regs.read(rs1).cast_signed();
            let divisor = regs.read(rs2).cast_signed();
            let value = if divisor == 0 {
                dividend
        ...
        MExtInstruction::Remu { rd, rs1, rs2 } => {
            let dividend = regs.read(rs1);
            let divisor = regs.read(rs2);
            let value = if divisor == 0 {
                dividend
