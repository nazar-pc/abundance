...
#[instruction]
impl<Reg> const Instruction for Rv64ZcaInstruction<Reg>
where
    Reg: [const] Register<Type = u64>,
{
    ...
    fn try_decode(instruction: u32) -> Option<Self> {
        ...
        /// Reconstruct the CJ-type jump offset used by C.J.
        ///
        /// Bit layout:
        /// ```text
        ///   imm[11]  = inst[12]
        ...
        match quadrant {
            ...
            0b00 => match funct3 {
                ...
                0b011 => {
                    let uimm5_3 = ((inst >> 10u8) & 0b111) as u8;
                    let uimm7_6 = ((inst >> 5u8) & 0b11) as u8;
                    let uimm = (uimm7_6 << 6u8) | (uimm5_3 << 3u8);
                    let rs1_bits = prime_reg_bits(((inst >> 7u8) & 0b111) as u8);
                    let rd_bits = prime_reg_bits(((inst >> 2u8) & 0b111) as u8);
                    let rs1 = Reg::from_bits(rs1_bits)?;
                    let rd = Reg::from_bits(rd_bits)?;
                    Some(Self::CLd { rd, rs1, uimm })
                }
                ...
                0b111 => {
                    ...
                    let rs1 = Reg::from_bits(rs1_bits)?;
                    let rs2 = Reg::from_bits(rs2_bits)?;
                    Some(Self::CSd { rs1, rs2, uimm })
                }
                // funct3 = 001/100/101 are reserved in Zca Q00 (used by Zcb)
                _ => None,
            },
            ...
            0b01 => match funct3 {
                ...
                // C.SRLI / C.SRAI / C.ANDI / arithmetic pairs
                0b100 => {
                    let funct2 = ((inst >> 10u8) & 0b11) as u8;
                    let rd_bits = prime_reg_bits(((inst >> 7u8) & 0b111) as u8);
                    match funct2 {
                        // C.SRLI  shamt[5]=inst[12], shamt[4:0]=inst[6:2]
                        ...
                        // Arithmetic: sub-selected by inst[12] and inst[6:5]
                        0b11 => {
                            ...
                            if bit12 == 0 {
                                ...
                                // funct2b=10/11 reserved in Zca
                                match funct2b {
                                    0b00 => Some(Self::CSubw { rd, rs2 }),
                                    0b01 => Some(Self::CAddw { rd, rs2 }),
                                    // 10/11 used by Zcb
                                    _ => None,
                                }
                            }
            ...
            // Quadrant 10
            0b10 => match funct3 {
                // C.SLLI  shamt[5]=inst[12], shamt[4:0]=inst[6:2]
                ...
                // C.LDSP  uimm[5]=inst[12], uimm[4:3]=inst[6:5], uimm[8:6]=inst[4:2]
                // rd=x0 is reserved
                0b011 => {
                    let rd_bits = ((inst >> 7u8) & 0x1f) as u8;
                    if rd_bits == 0 {
                    ...
                    let rd = Reg::from_bits(rd_bits)?;
                    let uimm5 = (inst >> 12u8) & 1;
                    let uimm43 = (inst >> 5u8) & 0b11;
                    let uimm86 = (inst >> 2u8) & 0b111;
                    let uimm = (uimm86 << 6u8) | (uimm5 << 5u8) | (uimm43 << 3u8);
                    Some(Self::CLdsp { rd, uimm })
                }
                ...
                // C.SDSP  uimm[5:3]=inst[12:10], uimm[8:6]=inst[9:7]
                0b111 => {
                    let rs2_bits = ((inst >> 2u8) & 0x1f) as u8;
                    let rs2 = Reg::from_bits(rs2_bits)?;
                    let uimm53 = (inst >> 10u8) & 0b111;
                    let uimm86 = (inst >> 7u8) & 0b111;
                    let uimm = (uimm86 << 6u8) | (uimm53 << 3u8);
                    Some(Self::CSdsp { rs2, uimm })
                }
                _ => None,
            },
    ...
    Reg: fmt::Display,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::CLd { rd, rs1, uimm } => write!(f, "c.ld {rd}, {uimm}({rs1})"),
            Self::CSw { rs1, rs2, uimm } => write!(f, "c.sw {rs2}, {uimm}({rs1})"),
            Self::CSd { rs1, rs2, uimm } => write!(f, "c.sd {rs2}, {uimm}({rs1})"),
            Self::CNop => write!(f, "c.nop"),
            Self::CAddi { rd, nzimm } => write!(f, "c.addi {rd}, {nzimm}"),
            Self::CAddiw { rd, imm } => write!(f, "c.addiw {rd}, {imm}"),
            ...
            Self::CSubw { rd, rs2 } => write!(f, "c.subw {rd}, {rs2}"),
            Self::CAddw { rd, rs2 } => write!(f, "c.addw {rd}, {rs2}"),
            ...
            Self::CLdsp { rd, uimm } => write!(f, "c.ldsp {rd}, {uimm}(sp)"),
