            ...
            match maybe_last_segment_index {
                Some(last_segment_index) => {
                    ...
                    if segment_index != last_segment_index + SegmentIndex::ONE {
                        let error = format!(
                            "Segment index {segment_index} must strictly follow \
    ...
    AS: AuxStore + Send + Sync + 'static,
{
    ...
    if parent_block_hash != best_archived_block_hash {
        let error = format!(
            "Attempt to switch to a different fork beyond archiving depth, can't do it: parent \
            block hash {parent_block_hash}, best archived block hash {best_archived_block_hash}"
        );
        return Err(sp_blockchain::Error::Consensus(sp_consensus::Error::Other(
            error.into(),
        )));
